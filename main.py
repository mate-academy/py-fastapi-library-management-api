...
def get_authors(skip: int = Query(0), limit: int = Query(10), db: Session = Depends(get_db)) -> List[schemas.Author]:
...
def get_books(skip: int = Query(0), limit: int = Query(10), db: Session = Depends(get_db)) -> List[schemas.Book]:
