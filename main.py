...
def read_single_author(author_id: int, db: Session = Depends(get_db)) -> Optional[schemas.Author]:
    library = Library(db)
...
def read_authors(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> list[schemas.Author]:
    library = Library(db)
    authors = library.get_book_list(skip=skip, limit=limit)
    return authors
...
def read_books_by_author(
    ...
    library = Library(db)
    books = library.get_books_by_author(author_id=author_id, skip=skip, limit=limit)
    if not books:
