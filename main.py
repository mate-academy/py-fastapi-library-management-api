...
def create_author(author: schemas.AuthorCreate, db: Session = Depends(get_db)):
    ...
    if author_name:
        raise HTTPException(
            status_code=400, detail="This author is already in our base"
        )
...
def read_books(
    ...
    return crud.get_all_books(
        db=db, author_id=author_id, page=page, page_size=page_size
    )
...
def read_detail_book(book_id: int, db: Session = Depends(get_db)):
    db_book = crud.get_detailed_book(db=db, book_id=book_id)
    if db_book:
