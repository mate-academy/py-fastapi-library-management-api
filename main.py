...
def read_all_authors(skip: int = 0,
                     limit: int = 10,
                     db: Session = Depends(get_db)) -> list[schemas.Author]:
    authors = crud.get_all_authors_with_pagination(db=db,
                                                   skip=skip,
                                                   limit=limit)
    return authors
...
def read_single_author(author_id: int,
                       db: Session = Depends(get_db)) -> schemas.Author:
...
def read_all_books(author_id: int = None,
                   skip: int = 0,
                   limit: int = 10,
...
def create_book(book: schemas.BookCreate,
    ...
    if db_author is None:
        raise HTTPException(status_code=404,
                            detail="Author with this id isn't found")
