...
def get_all_authors(db: Session, skip: int = 0, limit: int = 10) -> list[DBAuthor]:
...
def get_all_books(db: Session, skip: int = 0, limit: int = 10) -> list[DBBook]:
...
def create_book_for_author(db: Session, author_id: int, book: BookCreateForAuthor) -> DBBook:
    db_book = DBBook(
        title=book.title,
        summary=book.summary,
        published_date=book.published_date,
