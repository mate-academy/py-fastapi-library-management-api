...
def get_author_by_id(db: Session, pk: int):
    return (
        db.query(models.DBAuthor).filter(models.DBAuthor.id == pk).first()
    )
...
def read_authors_list(db: Session):
    return db.query(models.DBAuthor).all()

def create_book_for_specific_author(db: Session, book: BookCreate, name: str):
...
def get_books_with_pagination(db: Session, skip: int = 0, limit: int = 0):
...
def get_filtered_books_by_author_id(db: Session, author_id: int):
