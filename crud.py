...
def create_author(db: Session, author: schemas.AuthorCreateUpdate):
...
def get_authors(db: Session, page: int = 0, page_size: int = 100):
    return db.query(models.Author).offset(page).limit(page_size).all()


def get_detailed_author(db: Session, author_id: int):
...
def delete_author(db: Session, author_id_to_delete: int):
...
def create_book(db: Session, book_to_create: schemas.BookCreateUpdate):
...
def get_books(db: Session, page: int = 0, page_size: int = 100):
    return db.query(models.Book).offset(page).limit(page_size).all()


def get_detailed_book(db: Session, book_id: int):
